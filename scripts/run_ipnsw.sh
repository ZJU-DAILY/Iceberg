...
mode="search"
efc=256
efs=(100 200 300 400 450 500 600 800 1000 1500 2000 3000)
# efs=(2000 3000)
M=32
type="ip"

INDEX_PREFIX_PATH="${pre_path}/${algorithm}/${PREFIX}_M${M}_L${efc}.index_mag"
...
case "$mode" in
  build)
    echo "Building index..."
    ./test/benchmark_ipnsw ${BASE_PATH} ${QUERY_FILE} ${mode} $DATA_DIM $K ${efc} ${M} ${INDEX_PREFIX_PATH} ${RESULT_PREFIX_PATH} "$log_file"
    ;;
  search)
    ...
    for ef_search in "${efs[@]}"; do
      ...
      ./test/benchmark_ipnsw ${BASE_PATH} ${QUERY_FILE} ${mode} ${DATA_DIM} ${K} ${efc} ${M} ${INDEX_PREFIX_PATH} ${RESULT_PREFIX_PATH} ${ef_search} | tee -a "$log_file"
