...
algorithm="rabitq"
mode="build"
efs=(1 2 4 5 8 10 20 30 50 60 100 200 300 500)
efc=4096
M=${DATA_DIM}
type="nn"
...
case "$mode" in
  build)
    echo "Building index..."
    ./test/benchmark_rabitq ${BASE_PATH} ${QUERY_FILE} ${mode} $DATA_DIM $K ${efc} ${M} ${INDEX_PREFIX_PATH} ${RESULT_PREFIX_PATH} ${DATASET_NAME}  | tee -a "$log_file"
    ;;
  search)
    ...
    for ef_search in "${efs[@]}"; do
      ...
      ./test/benchmark_rabitq ${BASE_PATH} ${QUERY_FILE} ${mode} ${DATA_DIM} ${K} ${efc} ${M} ${INDEX_PREFIX_PATH} ${RESULT_PREFIX_PATH} ${DATASET_NAME} ${ef_search} | tee -a "$log_file"
