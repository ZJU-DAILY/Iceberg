...
public:
    ...
    size_t N;                       // the number of data vectors
    size_t C;                       // the number of clusters
    ...
    IVFRN(size_t D, size_t B);
    IVFRN(size_t D, size_t B,
        const Matrix<float> &X, const Matrix<float> &_centroids, const Matrix<float> &dist_to_centroid,
...
void IVFRN::fast_scan(ResultHeap &KNNs, float &distK, uint32_t k, \
    ...
    for(size_t i=0;i<B/4*16;i++)LUT[i] *= 2;
...
void IVFRN::load(char * filename){
    ...
    for(size_t i=0;i<B;i++)u[i] = uniform(gen);
#else
    for(size_t i=0;i<B;i++)u[i] = 0.5;
#endif
...
// template <uint32_t D, uint32_t B>
IVFRN::IVFRN(size_t DD, size_t BB){
    D = DD;
    B = BB;
    fac_norm = const_sqrt(1.0 * B);
    max_x1 = 1.9 / const_sqrt(1.0 * B-1.0);
    space.D = D;
    space.B = B;
    N = C = 0;
    start = len = id = NULL;
