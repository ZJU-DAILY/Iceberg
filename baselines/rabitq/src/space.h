...
public:
    ...
    inline void quantize(uint8_t *result, float *q, float* c, float *u, float max_entry, float width, uint32_t & sum_q) const;
    inline uint32_t sum(uint8_t* d);
    Space() {}
    Space(u_int32_t D, u_int32_t B): D(D), B(B) {};
    ~Space(){};
};
...
// template<uint32_t L>
inline float sqr_dist(uint32_t L, float *d, float *q) {
