...
#include <sstream>
#include <string>
#include "vamana/tsl/robin_set.h"
#include "stdio.h"
#include <unordered_map>
#include <fcntl.h>
#include <sys/stat.h>
#include <time.h>
#include "vamana/logger.h"
#include "vamana/exceptions.h"
#include "vamana/index.h"
#include "vamana/math_utils.h"
#include "vamana/memory_mapper.h"
#include "vamana/parameters.h"
#include "vamana/partition_and_pq.h"
#include "vamana/timer.h"
#include "vamana/utils.h"
#include <stack>

#include "vamana/distance.h"
#include "vamana/neighbor.h"
...
namespace diskann {
  ...
  class Index {
   public:
    Index(Metric m, const char *filename,
                            ...
                            : _metric(m), _num_frozen_pts(num_frozen_pts),_nd(num_data),_dim(dim),_has_built(false),
                    ...
                    //    load_aligned_bin<T>(std::string(filename), _data, _nd, _dim, _aligned_dim);
                          load_data<T>(std::string(filename), _data, _nd, _dim, _aligned_dim);
                        if (nd > 0) {
                          if (_nd >= nd)
    ...
    void save(const char *filename) {
      ...
      //           std::cout << "Wrong new_location assigned to  " << i
      //                         << std::endl;
      //         else {
      //           if ((_delete_set.find(i) != _delete_set.end()) &&
      //               (new_location[i] < _max_points + _num_frozen_pts))
    ...
    void load(const char *filename,
                                ...
                                const char *tag_filename = NULL){
                                  ...
                                  std::cout <<  "max length: " << _width << std::endl;
                                  ...
                                  while (!in.eof()) {
                                   ...
                                   // cout << "\n "<<nodes <<" : ";
                                    //for( int j = 0 ; j<k;j++)std::cout <<tmp[j] <<" " <<std::endl;
                                    //exit(1);
    ...
    int generate_random_frozen_points(
        const char *filename = NULL) {
          if (_has_built) {
            std::cout << "Index already built. Cannot add more points"
                          << std::endl;
            return -1;
          }
    ...
    void search(const T *      query,
            ...
            unsigned int _NDC = retval.second;
            unsigned int _NHOPS = retval.first;
            size_t pos = 0;
            ...
            // for (auto it : best_L_nodes) {
            //   indices[pos] = it.id;
            //   dist = _distance->compare(query,
            //                             _data + _aligned_dim * (size_t) it.id,
    ...
    std::pair<uint32_t, uint32_t> search(
    ...
    std::pair<uint32_t, uint32_t> search_with_tags(
    ...
    void readjust_data(unsigned _num_frozen_pts) {
      if (_num_frozen_pts > 0) {
        if (_final_graph[_max_points].empty()) {
          ...
          std::cout << "Finished updating graph, updating data now"
                        << std::endl;
          for (unsigned i = 0; i < _num_frozen_pts; i++) {
        ...
        std::cout << "No frozen points. No re-adjustment required"
                      << std::endl;
    }

    /* insertions possible only when id corresponding to tag does not already
     * exist in the graph */
    int insert_point(const T *                    point,
    ...
    int eager_delete(const TagT        tag,
                                       const Parameters &parameters) {
    ...
    void optimize_graph() {  // use after build or load
    ...
    void search_with_opt_graph(const T *query, size_t K,
  ...
  #pragma omp parallel for schedule(static, 65536)
    ...
    void update_in_graph() {
      std::cout << "Updating in_graph....." << std::flush;
    ...
    std::pair<uint32_t, uint32_t> iterate_to_fixed_point(
        ...
        std::vector<Neighbor> &      best_L_nodes) {
          ...
          for (auto id : init_ids) {
            ...
            dist = _distance->compare(_data + _aligned_dim * (size_t) id,
              ...
              for (unsigned m = 0; m < _final_graph[n].size(); ++m) {
                ...
                if (inserted_into_pool.find(id) == inserted_into_pool.end()) {
                  ...
                  if (dist >= best_L_nodes[l - 1].distance && (l == Lsize))
                    continue;
      /*
                  PRINTVEC(node_coords,_aligned_dim)
                  PRINTVEC(_data + _aligned_dim * (size_t) id,_aligned_dim)
                  PRINTDIST(dist)
                  EXIT_PRG
      */
                  Neighbor nn(id, dist, true);
  ...
  #pragma omp parallel for schedule(dynamic, 65536)
    ...
    size_t consolidate_deletes(const Parameters &parameters) {
      if (_eager_done) {
        std::cout << "No consolidation required, eager deletes done"
                      << std::endl;
        return 0;
      }
