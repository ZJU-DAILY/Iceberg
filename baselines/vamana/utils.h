...
#define PRINTDIST(dist) printf("%f\n",dist);
#define PRINTVEC(v,d) for(int ii=0;ii<d;ii++){ printf("%f ",*(v+ii)) ;};std::cout<<std::endl;
#define PRINTVEC2(v,d)printf("%f %f\n",*(v),*(v+d-1)) ;
#define EXIT_PRG exit(-1);


#include "vamana/logger.h"
#include "vamana/common_includes.h"
#include "vamana/cached_io.h"
...
namespace diskann {
  ...
  inline void get_bin_metadata(MemoryMappedFiles& files,
                               ...
                               size_t& ncols) {
    std::cout << "Getting metadata for file: " << bin_file << std::endl;
  ...
  inline void load_range_truthset(const std::string& bin_file, std::vector<std::vector<_u32>> &groundtruth, _u64 & gt_num) {
    ...
    std::cout << "Metadata: #pts = " << gt_num << ", #total_results = " << total_res << "..."
                  << std::endl;
  ...
  inline void load_aligned_bin_impl(std::basic_istream<char>& reader,
    ...
    size_t expected_actual_file_size =
        npts * dim * sizeof(T) + npts * sizeof(uint32_t);
    if (actual_file_size != expected_actual_file_size) {
      std::stringstream stream;
      stream << "Error. File size mismatch. Actual size is " << actual_file_size
             << " while expected size is  " << expected_actual_file_size
    ...
    for (size_t i = 0; i < npts; i++) {
      int32_t dummy;
      reader.read(reinterpret_cast<char*>(&dummy), sizeof(uint32_t)); 
    ...
    template<typename T>
    void load_data(const std::string &path, T*& data, const size_t npts, const int dim, size_t& rounded_dim){
        std::ifstream in(path.c_str(), std::ios::binary);
        if(!in.is_open()){std::cout<<"open file error, cannot be opened!"<<std::endl;exit(-1);}
//        rounded_dim = dim;
        rounded_dim = ROUND_UP(dim, 8);
        size_t allocSize = npts * rounded_dim * sizeof(T);
        alloc_aligned(((void**) &data), allocSize, 8 * sizeof(T));
        for (size_t i = 0; i < npts; i++) {
            ...
            in.read((char*) (data + i * rounded_dim), dim * sizeof(T));
            memset(data + i * rounded_dim + dim, 0, (rounded_dim - dim) * sizeof(T));
        }
        in.close();
    }
