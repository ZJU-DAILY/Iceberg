...
#ifndef PARTITION_PQ_H
#define PARTITION_PQ_H
...
#include <time.h>
#include <typeinfo>
#include <vamana/tsl/robin_map.h>
...
int retrieve_shard_data_from_ids(const std::string data_file,
                                 ...
                                 std::string       data_filename) {
                                    ...
                                    std::cout << "Written file with " << num_written << " points"
                                                  << std::endl;
...
int generate_pq_pivots(
    const float *passed_train_data, size_t num_train, unsigned dim,
    ...
    std::string pq_pivots_path, bool make_zero_mean = false) {
        if (num_pq_chunks > dim) {
          std::cout << " Error: number of chunks more than dimension"
                        << std::endl;
          return -1;
        }
        ...
        if (file_exists(pq_pivots_path)) {
          ...
          if (file_dim == dim && file_num_centers == num_centers) {
            std::cout << "PQ pivot file exists. Not generating again"
                          << std::endl;
            return -1;
          }
        }
