   ...
   public:
    ...
    if (file_exists(chunk_offset_file)) {
      ...
      if (numr != ndims_u64 || numc != 1) {
        std::cerr << "Error loading rearrangement file" << std::endl;
      ...
      if (numc != 1 || (numr != num_chunks + 1 && num_chunks != 0)) {
        std::cerr << "Error loading chunk offsets file. numc: " << numc
      ...
      if (numc != 1 || numr != ndims_u64) {
        std::cerr << "Error loading centroid file" << std::endl;
    ...
    std::cout << "PQ Pivots: #ctrs: " << npts_u64
